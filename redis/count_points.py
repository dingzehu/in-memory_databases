...
start_time = str('2015-10-12')
format = "%Y-%m-%d"
start_date = date(2015, 10, 12)
end_date = date(2021, 9, 30)
#end_date = date.today()-timedelta(1900)
...
while i < delta_date:
    if i ==1:
        ...
        #final_result = r.range('sensor', 0, -1)
...
#for b in final_result:
    #print(pickle.loads(b))
        ...
        entry_count.append(len(results))
...
#print("InfluxDB query: %d seconds" % (influx_query_end_time - influx_query_start_time))
        ...
        entry_count.append(len(results_later))
        
        print("accumulated number of entry until %s: %s" % (end, sum(entry_count)))

        continue
print("total number of entries: %s" % sum(entry_count))
